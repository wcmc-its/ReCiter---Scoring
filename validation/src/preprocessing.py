...
DERIVED_FEATURES_IDENTITY_SHARED = [
    ...
    # --- Phase 9 candidate features (identity-only, shared by both models) ---
    'emailAffilAgreement',         # email positive * best affiliation positive -- both identity anchors agree
    'grantOrgInteraction',         # grantMatchScore positive * orgUnit positive -- department + funding agreement
    'nameAffilStrength',           # nameMatchFirstScore * identityStrength -- good name match + strong identity
    'evidenceConsistency',         # std dev of positive evidence features -- lower = more consistent evidence
    # --- Phase 9 Round 2 candidate features (identity-specific interactions) ---
    'nameMatchConsistency',        # nameFirst * nameLast * nameMiddle -- all three name components agree
    'affilRelationshipInteraction',# max(targetAffil, pubmedAffil) * relationshipPositive -- affil AND relationships agree
    'scopusPubmedAffilAgreement',  # scopusAffil * pubmedAffil -- both affiliation sources agree
]
...
DERIVED_FEATURES_FEEDBACK = [
    ...
    # --- Phase 9 candidate features (feedback-only, BibCoupling + aggregation extensions) ---
    # BibCoupling has high correlation with Journal (0.71) and CoAuthorName (0.65).
    # Its standalone AUC is 0.661 overall but drops to 0.492 at low feedback (0-5 bucket).
    # These interaction features condition BibCoupling on feedback volume to reduce
    # noise at cold-start while preserving signal for established researchers.
    'bibCouplingFeedbackConfInteraction',  # bibCoupling * feedbackConfidence -- continuous attenuation
    'bibCouplingHighConfOnly',     # bibCoupling gated by minimum feedback threshold (~10 articles)
    'hasBibCouplingSignal',        # Binary: bibCoupling != 0 -- 88% coverage, 12% zero may carry info
    'worstSingleEvidenceExtended', # worstSingleEvidence expanded with textSim dimension
    'netEvidenceCountExtended',    # netEvidenceCount expanded with textSim + bibCoupling dimensions
    # NOTE: feedbackDensityExtended NOT NEEDED -- feedbackDensity already counts all 15
    # feedback features including textSim, JTS, and bibCoupling.
] + DERIVED_FEATURES_IDENTITY_SHARED
...
# Ablation variant control — set _ABLATION_EXCLUDE before importing to test subsets.
# Usage: preprocessing._ABLATION_EXCLUDE = {'featureA', 'featureB'}; then re-derive lists.
# Or set via environment: ABLATION_EXCLUDE=featureA,featureB
import os as _os
_ABLATION_EXCLUDE = set(
    f.strip() for f in _os.environ.get('ABLATION_EXCLUDE', '').split(',') if f.strip()
)

# Complete feature lists (filtered by ablation exclusion set)
FEEDBACK_IDENTITY_FEATURES = [
    f for f in FEEDBACK_IDENTITY_BASE_FEATURES + DERIVED_FEATURES_FEEDBACK
    if f not in _ABLATION_EXCLUDE
]
IDENTITY_ONLY_FEATURES = [
    f for f in IDENTITY_ONLY_BASE_FEATURES + DERIVED_FEATURES_IDENTITY_ONLY
    if f not in _ABLATION_EXCLUDE
]
...
def reconfigure_features(exclude: set = None):
    """Reconfigure FEEDBACK_IDENTITY_FEATURES and IDENTITY_ONLY_FEATURES
    after changing _ABLATION_EXCLUDE. Call this from evaluation scripts
    to test different feature subsets without restarting.

    Args:
        exclude: Set of feature names to exclude. If None, uses current _ABLATION_EXCLUDE.
    """
    global FEEDBACK_IDENTITY_FEATURES, IDENTITY_ONLY_FEATURES, _ABLATION_EXCLUDE
    if exclude is not None:
        _ABLATION_EXCLUDE = set(exclude)
    FEEDBACK_IDENTITY_FEATURES = [
        f for f in FEEDBACK_IDENTITY_BASE_FEATURES + DERIVED_FEATURES_FEEDBACK
        if f not in _ABLATION_EXCLUDE
    ]
    ...
    return len(FEEDBACK_IDENTITY_FEATURES), len(IDENTITY_ONLY_FEATURES)
...
# Extended evidence list for netEvidenceCountExtended (Phase 9 candidate).
# Adds textSimilarity and bibCoupling to the positive/negative signal count.
# Do NOT modify _POSITIVE_EVIDENCE_FEATURES itself — this is a new separate list.
...
def _compute_identity_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 24. emailAffilAgreement: email and affiliation both present and positive.
    #     Both identity anchors agreeing is a stronger signal than either alone.
    best_affil_cand = np.maximum(
        df['targetAuthorInstitutionalAffiliationMatchTypeScore'],
        df['pubmedTargetAuthorInstitutionalAffiliationMatchTypeScore']
    )
    df['emailAffilAgreement'] = (
        df['emailMatchScore'].clip(lower=0) * best_affil_cand.clip(lower=0)
    )

    # 25. grantOrgInteraction: grant and org unit both match.
    #     Department-level + funding agreement is strong identity evidence.
    df['grantOrgInteraction'] = (
        df['grantMatchScore'].clip(lower=0) * df['organizationalUnitMatchingScore'].clip(lower=0)
    )

    # 26. nameAffilStrength: name match weighted by overall identity strength.
    #     Good name match means more when identity anchors are also strong.
    df['nameAffilStrength'] = df['nameMatchFirstScore'] * df['identityStrength']

    # 27. evidenceConsistency: standard deviation of positive evidence features.
    #     Lower variance = more consistent evidence across multiple dimensions.
    #     Consistent positive evidence is more trustworthy than one strong + many weak.
    df['evidenceConsistency'] = df[_POSITIVE_EVIDENCE_FEATURES].std(axis=1)

    # --- Phase 9 Round 2 candidate features (identity-specific interactions) ---

    # 28. nameMatchConsistency: product of all three name components.
    #     When first, last, AND middle all agree, the signal is much stronger.
    #     Triples concordant signals, nullifies if any component is zero or discordant.
    df['nameMatchConsistency'] = (
        df['nameMatchFirstScore'] * df['nameMatchLastScore'] * df['nameMatchMiddleScore']
    )

    # 29. affilRelationshipInteraction: best affiliation * relationship positive.
    #     When both institutional affiliation and personal relationships agree,
    #     the combined signal is stronger than either alone.
    ...
    # 30. scopusPubmedAffilAgreement: both affiliation sources agree.
    #     Scopus and PubMed independently providing positive affiliation is
...
def compute_derived_features_feedback_identity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- Phase 9 candidate features: BibCoupling interaction + aggregation extensions ---
    bib_coupling = df['feedbackScoreBibliographicCoupling']

    # 15. bibCouplingFeedbackConfInteraction: BibCoupling scaled by feedback confidence.
    #     BibCoupling AUC is 0.492 at low feedback but 0.661 at high feedback.
    #     Multiplying by feedbackConfidence naturally attenuates the signal for
    #     cold-start users where BibCoupling is unreliable.
    df['bibCouplingFeedbackConfInteraction'] = bib_coupling * df['feedbackConfidence']

    # 16. bibCouplingHighConfOnly: BibCoupling gated by minimum feedback threshold.
    #     Binary gate that activates BibCoupling only when ~10+ feedback articles exist.
    #     Let XGBoost compare continuous vs binary gating.
    df['bibCouplingHighConfOnly'] = (
        bib_coupling * (df['feedbackConfidence'] > np.log1p(10)).astype(float)
    )

    # 17. hasBibCouplingSignal: binary presence indicator for BibCoupling.
    #     88% coverage means 12% zero values may carry information.
    df['hasBibCouplingSignal'] = (bib_coupling != 0).astype(float)

    # 18. worstSingleEvidenceExtended: expand worstSingleEvidence to include textSim.
    #     TextSim has negative correlations with top features (-0.33 CoAuthorName).
    #     When textSim is present and very negative, it could be the worst evidence.
    df['worstSingleEvidenceExtended'] = np.minimum(
        df['worstSingleEvidence'],
        np.where(text_sim != 0, text_sim, df['worstSingleEvidence'])
    )

    # 19. netEvidenceCountExtended: expand evidence count with textSim + bibCoupling.
    #     Uses extended feature lists that add the two new feedback dimensions.
    ext_pos_count = sum(
